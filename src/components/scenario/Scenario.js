      ...
      const columns = [
        {
          ...
          render: (text, record) => <Link to={`/${record.isSimulatorOptimiser === 'Optimizer' ? 'optimizer' : 'simulator'}/${record.id}/${record.model}/${record.geography}/${record.isSimulated ? `Simulated` : ''}`}>{text}</Link>,
